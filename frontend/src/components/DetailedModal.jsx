...
const DetailedModal = ({ item, onClose, layoutId }) => {
  ...
  // We use the passed layoutId, or fallback to the standard project ID format
  const activeLayoutId = layoutId || `card-${item.id}`;
  ...
  return (
    ...
    >
      <motion.div 
        // THIS IS THE MAGIC LINE: It now matches whatever card was clicked
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <div className="lg:col-span-8 space-y-10">
              ...
              <div className="space-y-4">
                <h3 className="text-sm font-mono text-blue-500 uppercase tracking-[0.3em]">Overview_Data</h3>
                <p className="text-slate-300 text-lg leading-relaxed font-light">
                  {item.long_description || (item.points && item.points[0]) || "No detailed description available."}
                </p>
              </div>
              ...
              {item.roadmap && (
                <div className="p-8 bg-slate-800/20 rounded-3xl border border-slate-800/50">
                  <h3 className="text-xs font-mono text-slate-500 uppercase tracking-widest mb-8">Execution_Roadmap</h3>
            ...
            {/* RIGHT COLUMN: Technical Specs & Links */}
